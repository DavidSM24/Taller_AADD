...
public class CarRepairService {
	...
	/**
	 * M�todo que devuelve una lista con todas las reparaciones de la base de
	 * datos
	 * 
		...
		if (carRepair != null) {
			if (carRepair.getId() != null && carRepair.getId() > 0) {
				...
				if (aux.isPresent()) {
					...
					throw new RecordNotFoundException("El id introducido no se encuentra en la base de datos",
							carRepair.getId());
				}
			} else {
	...
	public CarRepair createOrUpdateCarRepair(CarRepair carRepair) throws ServiceException {
		if (carRepair != null) {
			if (carRepair.getAmount() > -1 && carRepair.getAsigPoints() > -1
					&& (carRepair.getBrandCar() != null
							&& (carRepair.getCarPlate().length() > 4 && carRepair.getCarPlate().length() < 11))
				...
				if (carRepair.getId() != null && carRepair.getId() > 0) {// si tiene id y es mayor de 0
					...
					if (result.isPresent()) {// si lo encuentra en la base de datos
						...
						if (!newCarRepair.isRepaired() && carRepair.isRepaired()) {
							sum(carRepair.getMyAgency(), carRepair.getAsigPoints(),carRepair.getAmount());
						}
						
						else if(!carRepair.isRepaired()&& newCarRepair.isRepaired()) {
							rest(carRepair.getMyAgency(), carRepair.getAsigPoints(),carRepair.getAmount());
						}
	...
	/**
	 * M�todo que devuelve una lista de reparaci�nes paginada seg�n el cliente
	 * del coche
	 * 
	 * @param name
	 * @param nElement
	...
	/**
	 * M�todo que devuelve todas las reparaciones paginadas seg�n la fecha de
	 * entrada
	 * 
	 * @param ini
	 * @param end
	 ...
	 * @throws ServiceException
	 */
	public List<CarRepair> getByDateOrderPaged(LocalDateTime ini, LocalDateTime end, int nELement, int paged)
			throws ServiceException {
	 ...
	 * @throws ServiceException
	 */
	public boolean sum(Agency agency, long points, float f) throws ServiceException {
		if (agency != null) {
			if (agency.getId() != null && agency.getId() > 0) {
				if (points > 0 && f >0) {
					...
					newAgency.setAmount(newAgency.getAmount() + f);
	...
	public boolean rest(Agency agency, long f, float g) throws ServiceException {
		if (agency != null) {
			if (agency.getId() != null && agency.getId() > 0) {
				if (f > 0 && g >0) {
					...
					newAgency.setPoints(newAgency.getPoints() - f);
					newAgency.setAmount(newAgency.getAmount()-g);
					agencyService.createOrUpdate(newAgency);
	...
	/**
	 * M�todo que devuelve una lista con las reparaciones que pertenecen a una agencia
	 * @param id_agency id de la agencia que se quiera obtener las reparaciones
	 * @param nElement n�mero de reparaciones que se quiere obtener
	 * @param paged a partir de que elemento se empieza a contar
	 * @return List<CarRepair> reparaciones de una agencia en concreto
	 * @throws ServiceException
	 */
	public List<CarRepair> getByAgencyPaged(int id_agency, int nElement, int paged) throws ServiceException {
		if (id_agency > -1) {
			if (nElement > 0 && paged > -1) {
				logger.info("Petici�n realizada correctamente");

				return repository.getByAgencyPaged(id_agency, nElement, paged);
			...
			logger.error("El id introducido esta por debajo de 0");

			throw new ServiceException("El id introducido esta por debajo de 0");
		}
	}
