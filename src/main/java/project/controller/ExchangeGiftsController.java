...
public class ExchangeGiftsController {
	...
	/**
	 * Devuelve una respuesta HTTP con una lista de regalos intercambiados filtrados
	 * dependiendo del nombre del regalo.
	 *
	 * @param gname el parametro nombre para filtrar.
	 * @return la respuesta HTTP con la lista de regalos intercambiados.
	 */
	@ApiOperation(value = "Return all ExchangesGifts by gift names", notes="Return a ExchangesGifts List")
	@ApiResponses(value = {
			@ApiResponse(code=200,message="Successful Operation"),
			@ApiResponse(code=400,message="Bad Request"),
			@ApiResponse(code=404,message="ERROR, Can't get ExchangesGifts"),
	...
	@GetMapping("/gname/{gname}")
	public ResponseEntity<List<ExchangeGift>> getByGiftName(@PathVariable("gname") String gname){
		List<ExchangeGift> all;
		try {
			all = service.getByGiftName(gname);
	...
	/**
	 * Devuelve una respuesta HTTP con una lista de regalos intercambiados filtrados
	 * dependiendo del nombre del usuario de la agencia del canje.
	 *
	 * @param auname el parametro nombre para filtrar.
	 * @return la respuesta HTTP con la lista de regalos intercambiados.
	 */
	@ApiOperation(value = "Return all ExchangesGifts by agency user names", notes="Return a ExchangesGifts List")
	@ApiResponses(value = {
			@ApiResponse(code=200,message="Successful Operation"),
			@ApiResponse(code=400,message="Bad Request"),
			@ApiResponse(code=404,message="ERROR, Can't get ExchangesGifts"),
	...
	@GetMapping("/auname/{auname}")
	public ResponseEntity<List<ExchangeGift>> getByAUName(@PathVariable("auname") String auname){
		List<ExchangeGift> all;
		try {
			all = service.getByAUName(auname);
	...
	public HttpStatus delete(@Valid @RequestBody ExchangeGift ex) throws RecordNotFoundException{
		try {
			...
			ExchangeGift tmp=service.getbyId(ex.getId());
			
			if(tmp!=null&&service.delete(ex)) {

				this.service.sumPoints(tmp.getAgency(), tmp.getGift());
